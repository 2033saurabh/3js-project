...
gltfLoader.load(
  ...
  (gltf) => {
    ...
    donut.scale.set(0.5, 0.5, 0.5, 0.5);
  ...
  if (newSection != currentSection) {
    ...
    if (!!donut) {
      gsap.to(
        ...
        duration: 1.5,
        ease: 'power5.inOut',
        y: transformDonut[currentSection].rotationZ 
      } 
      )
      gsap.to(
        donut.position, {
      ...
      gsap.to(
        ...
        duration: 1.5,
        ease: 'power2.inOut',
        x: transformDonut[currentSection].positionX - 10.2
      }
      )
    }
  }
...
window.addEventListener('mousemove', (event) => {
  if (mouseDown) {
    ...
    mouseX = event.clientX;
    if (!!donut) {
      donut.rotation.y += deltaX * 0.01; // Adjust the rotation speed
    }
  }
});
...
window.addEventListener('mousemove', (event) => {
  if (mouseUp) {
    ...
    if (!!donut) {
      donut.rotation.x += deltaY * 0.01; // Adjust the rotation speed
    }
  }
});
...
const animate = () => {
  ...
  if (!!donut) {
    donut.position.y = Math.sin(elapsedTime * 0.5) * .1 - 0.1
